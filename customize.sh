...
# ── Підтвердження ────────────────────────────────────────
# v0.0.522: автооновлення з WebUI передає SKIP_DISCLAIMER=1,
# щоб пропустити інтерактивний вибір кнопками гучності (у фоновому
# режимі ніхто не натисне Volume+, тому inst кінцевого таймауту
# вибирає «Ні (скасувати)» і модуль не встановлюється).
if [ "${SKIP_DISCLAIMER:-0}" != "1" ]; then
