...
# ── Підтвердження ────────────────────────────────────────
# v0.0.525/0.0.526: пропускаємо інтерактивний вибір, якщо:
#   - SKIP_DISCLAIMER=1 (автооновлення з WebUI)
#   - UNATTENDED=1 (загальний неінтерактивний режим)
#   - [ ! -t 0 ] (відсутність інтерактивного tty)
#   - модуль вже встановлений (оновлення)
# У фоновому/неінтерактивному режимі ніхто не натисне Volume+,
# тому таймаут за замовчуванням вибирає «Ні» і скасовує встановлення.
MODPATH_EXIST="/data/adb/modules/td2tdr_sync"
IS_UPDATE=0
if [ -d "$MODPATH_EXIST" ]; then
  IS_UPDATE=1
fi
if [ "${SKIP_DISCLAIMER:-0}" != "1" ] && [ "${UNATTENDED:-0}" != "1" ] && [ -t 0 ] && [ "$IS_UPDATE" != "1" ]; then
  ...
  if [ "$IS_UPDATE" = "1" ]; then
    ui_print " "
    ui_print "ℹ️ Виявлено оновлення модуля. Пропускаємо дисклеймер."
    ui_print " "
  else
    ui_print " "
    ui_print "⚡ Автооновлення: пропускаю інтерактивний дисклеймер"
    ui_print " "
  fi
fi
